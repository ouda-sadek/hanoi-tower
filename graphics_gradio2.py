...
def render_pegs(moves, step, disk_count, peg_count):
    # Initialiser les tiges avec tous les disques sur la tige 1 (index 0)
    pegs = [[] for _ in range(peg_count)]
    for i in range(disk_count, 0, -1):  # disques de N à 1
        pegs[0].append(i)

    # Appliquer les mouvements jusqu'à l'étape donnée
