...
auto_moves = []
auto_timer = 0
auto_delay = 300  # milliseconds between moves
...
def reset():
    global steps,pointing_at,floating,floater,auto_mode,auto_moves
    steps = 0
    pointing_at = 0
    floating = False
    floater = 0
...
def execute_auto_move():
    ...
    new_top = 400 - 23
    for disk in disks[::-1]:
        if disk['tower'] == to_tower and disk != moving_disk:
            new_top = disk['rect'].top - 23
            break
    moving_disk['rect'].midtop = (towers_midx[to_tower], new_top)
    moving_disk['tower'] = to_tower
    steps += 1
